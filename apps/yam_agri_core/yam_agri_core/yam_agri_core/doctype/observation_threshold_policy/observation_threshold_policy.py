...
class ObservationThresholdPolicy(Document):
	...
	def validate(self):
		...
		if (
			...
			and float(critical_min) > float(critical_max)
		):
		...
		if warning_min is not None and critical_min is not None and float(critical_min) > float(warning_min):
			frappe.throw(
				_("Critical Min should be less than or equal to Warning Min"), frappe.ValidationError
			)

		if warning_max is not None and critical_max is not None and float(critical_max) < float(warning_max):
			frappe.throw(
				_("Critical Max should be greater than or equal to Warning Max"), frappe.ValidationError
			)
