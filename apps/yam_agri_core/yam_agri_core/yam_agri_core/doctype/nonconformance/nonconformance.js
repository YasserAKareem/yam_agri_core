function yamNcSubmitAiDecision(interactionLog, decision) {
	if (!interactionLog) {
		return;
	}
...
function yamNcPromptAiDecision(interactionLog) {
	if (!interactionLog) {
		return;
	}

	frappe.confirm(
		__("Do you accept this AI CAPA suggestion?"),
		() => yamNcSubmitAiDecision(interactionLog, "accepted"),
		() => yamNcSubmitAiDecision(interactionLog, "rejected")
	);
}

frappe.ui.form.on("Nonconformance", {
	refresh(frm) {
		if (frm.is_new()) {
			frm.set_intro(__("Save this Nonconformance first, then use AI CAPA Draft."), "orange");
			return;
		}
		frm.set_intro("");
		...
		frm.add_custom_button(
			...
			() => {
				frappe.call({
					method: "yam_agri_core.yam_agri_core.api.ai_assist.get_nonconformance_capa_suggestion",
					args: { nonconformance: frm.doc.name },
					freeze: true,
					freeze_message: __("Generating assistive CAPA draft..."),
					callback(r) {
						const msg = r.message || {};
						if (!msg.ok) {
							frappe.msgprint({
								title: __("AI CAPA Draft"),
						...
						const lotName = frappe.utils.escape_html(msg.lot || "");
						...
						frappe.msgprint({
							...
							message:
								...
								`<hr>` +
								`<div>${__("Nonconformance")}: ${frappe.utils.escape_html(frm.doc.name)}</div>` +
								`<div>${__("Lot")}: ${lotName || __("Not linked")}</div>` +
						...
						yamNcPromptAiDecision(msg.interaction_log || "");
					},
				});
			},
			__("AI")
