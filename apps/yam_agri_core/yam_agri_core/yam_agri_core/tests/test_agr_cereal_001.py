...
def test_recommend_returns_ranked_list():
    ...
    assert recs
    assert all(r.variety for r in recs)
    assert recs == sorted(recs, key=lambda r: (r.score, r.confidence), reverse=True)


def test_rotation_penalty_applies_when_same_crop():
    recs_ok = recommend(
        plot={"site": "S1", "last_crop": "Barley"},
        season="2026",
        crop="Wheat",
    ...
    assert recs_ok[0].predicted_yield_kg_per_ha >= recs_bad[0].predicted_yield_kg_per_ha
    assert recs_ok[0].score >= recs_bad[0].score
